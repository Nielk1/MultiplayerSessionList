    ...
    public class GameListModule : IGameListModuleOld
    {
        ...
        public async Task<GameListData> GetGameList(bool admin)
        {
            using (var http = new HttpClient())
            {
                ...
                DefaultSession.Attributes.Add(GAMELIST_TERMS_OLD.ATTRIBUTE_LISTSERVER, $"Rebellion");

                DataCacheOld Metadata = new DataCacheOld();
                if (res_raw.Content.Headers.LastModified.HasValue)
                    Metadata.AddObjectPath($"{GAMELIST_TERMS_OLD.ATTRIBUTE_LISTSERVER}:Rebellion:Timestamp", res_raw.Content.Headers.LastModified.Value.ToUniversalTime().UtcDateTime);

                DataCacheOld DataCache = new DataCacheOld();
                DataCacheOld Mods = new DataCacheOld();
                DataCacheOld Heroes = new DataCacheOld();
                ...
                */
                ...
                foreach (var raw in gamelist.Values)
                {
                    ...
                    Tasks.Add(Task.Run(async () =>
                    {
                        ...
                        game.PlayerCount.Add(GAMELIST_TERMS_OLD.PLAYERTYPE_PLAYER, raw.userCount);
                        ...
                        {
                            game.Status.Add(GAMELIST_TERMS_OLD.STATUS_LOCKED, true);
                        }
                        else
                        {
                        ...
                        game.Status.Add(GAMELIST_TERMS_OLD.STATUS_STATE, raw.IsEnded ? SESSION_STATE_LEGACY.PostGame : raw.IsLaunched ? SESSION_STATE_LEGACY.InGame : SESSION_STATE_LEGACY.PreGame);
                                ...
                                try
                                {
                                    if (!Heroes.ContainsPath($"{vehicle.Key.Replace(":", "\\:")}"))
                                    {
                                        ...
                                        if (vehicle.Value.description != null)
                                        {
                                            ...
                                            {
                                                Heroes.AddObjectPath($"{vehicle.Key.Replace(":", "\\:")}:Description", vehicle.Value.description["en"]);
                                            }
                                            else if (vehicle.Value.description.ContainsKey("default"))
                                            {
                                                Heroes.AddObjectPath($"{vehicle.Key.Replace(":", "\\:")}:Description", vehicle.Value.description["default"]);
                                            }
                                        }
                                    }
                                }
