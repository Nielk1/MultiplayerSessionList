...
public class GameListModule : IGameListModule
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var res = await cachedAdvancedWebClient.GetObject<Dictionary<string, Lobby>>(
            queryUrl,
...
//#endif
        ...
        foreach (var kv in BuildSources(res, cancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (Lobby raw in gamelist.Values)
        {
            ...
            Datum? session = TryCreateSessionDatum(raw);
            if (session == null)
                continue;

            if (raw.LobbyType == Lobby.ELobbyType.Game && raw.isPrivate && !(raw.IsPassworded ?? false))
                continue;

            ApplySessionBasics(raw, session);
            ApplySessionPlayerTypesAndCounts(raw, session);

            if (raw.LobbyType == Lobby.ELobbyType.Game)
            {
                foreach (var extraDatum in ApplyGameLobbyFields(raw, session, datumsAlreadyQueued, pendingWorkPool, fact_task, cancellationToken))
                {
                    cancellationToken.ThrowIfCancellationRequested();
            ...
            foreach (var playerDatum in BuildPlayerDatums(raw, admin, pendingWorkPool, players, cancellationToken))
            {
                cancellationToken.ThrowIfCancellationRequested();
                yield return playerDatum;
            }
    ...
    private IEnumerable<Datum> BuildPlayerDatums(Lobby raw, bool admin, DynamicAsyncEnumerablePool<Datum> pendingWorkPool, List<DatumRef> playerRefs, CancellationToken cancellationToken)
    {
        foreach (var dr in raw.users.Values)
        {
            ...
            if (raw.LobbyType == Lobby.ELobbyType.Chat && !string.IsNullOrEmpty(dr.id) && dr.id[0] == 'B')
            {
            ...
            playerRefs.Add(new DatumRef(GAMELIST_TERMS.TYPE_PLAYER, $"{GameID}:{dr.id}"));
        }
    }

    private static Datum? TryCreateSessionDatum(Lobby raw)
    {
        return raw.LobbyType switch
        {
            Lobby.ELobbyType.Chat => new Datum(GAMELIST_TERMS.TYPE_LOBBY, $"{GameID}:Rebellion:B{raw.id}"),
            Lobby.ELobbyType.Game => new Datum(GAMELIST_TERMS.TYPE_SESSION, $"{GameID}:Rebellion:B{raw.id}"),
            _ => null
        };
    }

    private static void ApplySessionBasics(Lobby raw, Datum session)
    {
    ...
    private IEnumerable<(string shortId, Datum data)> BuildSources(
        CachedData<Dictionary<string, Lobby>> rebellionResult,
        CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        CachedData<MapData>? mapDataC = await cachedAdvancedWebClient.GetObject<MapData>(
            $"{mapUrl}/getdata2.php?map={encodedMapId}&mods={encodedModId}",
            cancellationToken: cancellationToken);
